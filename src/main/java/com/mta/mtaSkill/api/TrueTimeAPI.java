...
public class TrueTimeAPI extends BaseAPIParser {
	 ...
	 *  rt- single route designator (required). Alphanumeric designator of the route (ex. â€œ20â€? or â€œX20â€?) for which a list of available stops is to be returned.
	 *  dir- single route direction (required). Direction of the route (ex. â€œEast Boundâ€?) for which a list of available stops is to be returned. This needs to match the direction in the getdirections call. When using multiple languages, it must match the direction for that language.
	 ...
	 * stpnm- Display name of this stop (ex.â€œMadison and Clarkâ€?)
	 ...
	 * @param tmres - string(optional) - Resolution of time stamps. Set to â€œsâ€? to get time resolution to the second. Set to â€œmâ€? to get time resolution to the minute. If omitted, defaults to â€œmâ€?. Date and time is represented in the following format: If specified as â€œsâ€? YYYYMMDD HH:MM:SS If specified as â€œmâ€? YYYYMMDD HH:MM Month is represented as two digits where January is equal to â€œ01â€? and December is equal to â€œ12â€?. Time is represented using a 24-hour clock.
	 * @return
	 * error - Message if the processing of the request resulted in an error.
	 ...
	 * rt - Alphanumeric designator of the route (ex. â€œ20â€? or â€œX20â€?) for which this prediction was generated.
	 * rtdd - Language-specific route designator meant for display.
	 * rtdir - Direction of travel of the route associated with this prediction (ex. â€œEast Boundâ€?). 
	 * des - Final destination of the vehicle associated with this prediction.
	 * prdtm - Predicted date and time (local) of a vehicleâ€™s arrival or departure to the stop associated with this prediction. Date and time is represented based on the tmres parameter.
	 * dly - â€œtrueâ€? if the vehicle is delayed. The dly element is only present if the vehicle that generated this prediction is delayed. 
	 * tablockid - TAâ€™s version of the scheduled block identifier for the work currently being performed by the vehicle.
	 * tatripid -TAâ€™s version of the scheduled trip identifier for the vehicleâ€™s current trip.
	 ...
	 * Date and time are represented in the following format: YYYYMMDD HH:MM:SS. Month is represented as two digits where January is â€œ01â€? and December is â€œ12â€?. Time is represented using a 24-hour clock. 
	 * 
	 */
	 ...
	 * tmstmp- Date and local time of the last positional update of the vehicle. Date and time is represented in the following format: YYYYMMDD HH:MM. Month is represented as two digits where January is equal to â€œ01â€? and December is equal to â€œ12â€?. Time is represented using a 24-hour clock.
	 ...
	 * rt- Route that is currently being executed by the vehicle (ex. â€œ20â€?).
	 * des- Destination of the trip being executed by the vehicle (ex. â€œAustinâ€?).
	 * dly- The value is â€œtrueâ€? if the vehicle is delayed. The dly element is only present if the vehicle is delayed.
	 ...
	 * Use rt to specify a route identifier where all active patterns are returned. The set of active patterns returned includes: one or more patterns marked as â€œdefaultâ€? patterns for the specified route and all patterns that are currently being executed by at least one vehicle on the specified route.
	 ...
	 * rtdir- Direction that is valid for the specified route designator. For example, â€œEast Boundâ€?.
	 ...
	 * typ- â€˜Sâ€™ if the point represents a Stop, â€˜Wâ€™ if the point represents a waypoint along the route.
